...
- Emit the `MAINTENANCE` error code (503) while maintenance mode is active — the body is the canonical envelope plus a `details` object (`{"reason":"{code}","self_healing":true}`), and the wait time and reason travel as the `Retry-After: 30`, `X-Maintenance-Mode: true`, and `X-Maintenance-Reason: {code}` headers, never as extra top-level body fields
