...
func TestCacheCapacity(t *testing.T) {
	...
	for i, tt := range tests {
		...
		var responses []model.IPLookupResponse
		for i := 0; i < tt.addCount; i++ {
			ip := net.ParseIP(fmt.Sprintf("192.0.2.%d", i))
			r := model.IPLookupResponse{IP: ip}
...
func TestCacheDuplicate(t *testing.T) {
	...
	response := model.IPLookupResponse{IP: ip}
	c.Set(ip, response)
	c.Set(ip, response)
	want := 1
...
func TestCacheResize(t *testing.T) {
	...
	r := model.IPLookupResponse{IP: net.ParseIP("192.0.2.42")}
	c.Set(r.IP, r)
